...
Step 5: 에이전트 자율 루프 시작
  ...
  → [로컬] "보너스 0.12 ROSE는 이의 제기 기간(24h) 후 클레임 가능."
  → [로컬] 다음 바운티 탐색...
```
...
| **PR 코멘트 배달** | 미구현 | 취약점 발견을 GitHub PR comment로 직접 전달 |
| `pora bounty watch` | 미구현 | 폴링 루프 + 완료 시 자동 retrieve |
| `pora audit dispute` | 미구현 | CLI 명령 추가 |

---

## 시나리오 C: 게으른 수행자가 시장을 악용한다

### 전제

이씨는 NoFindings만 제출해서 실행 수수료(40%)를 빨아먹으려 한다.
...
```
감사 1: NoFindings → 실행 수수료 수령 → score 변화 없음 (성공 처리)
...
- [ ] 수행자가 dispute 남발 요청자를 감지하고 해당 바운티를 거부할 수 있는가?
- [ ] dispute 해결 기한이 없으면 수행자 자금이 영구 동결되는지 확인

---

## PayoutPolicy 재설계 (CCG 합성 결과)

현재 40/30/20/10은 실행 수수료에 너무 치중됨. NoFindings에 같은 실행 수수료를 주면 시장이 스팸으로 죽는다.
...
FindingsFound인 경우:
  ...
  regression reserve: 15%  (이의 제기 기간)
...
- `_computePayout`에 취약점 발견 여부에 따른 분기 추가
- 컨트랙트 테스트 업데이트
