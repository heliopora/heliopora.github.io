...
- [VISION.md](VISION.md) — 목표 아키텍처 및 수행자 에이전트 모델
- [EVOLUTION_ROADMAP.md](EVOLUTION_ROADMAP.md) — 해결된 논쟁을 포함한 전략적 발전 계획
- [MARKET_PRINCIPLES.md](MARKET_PRINCIPLES.md) — 인센티브, 참여자 역할, 시장 설계
- [SIMULATION_TEST_PLAN.md](SIMULATION_TEST_PLAN.md) — 테스트넷 시뮬레이션 시나리오 및 레드팀 분석
- [ENCRYPTED_DELIVERY_ARCHITECTURE.md](ENCRYPTED_DELIVERY_ARCHITECTURE.md) — 요청자 전용 암호화 전달 설계
- [OASIS_ROFL_BLOCKERS.md](OASIS_ROFL_BLOCKERS.md) — 현재 Oasis/ROFL 운영 차단 이슈
...
- **브랜드: pora** — πόρος(통로). 코드가 들어가고 findings만 나오고 나머지는 사라지는 통로. 마스코트: heliopora(파란 산호).
